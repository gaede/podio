  ...
  #    for name, klass in components.iteritems():
    ...
    def create_sio(self, classname, definition):
      ...
      for item in definition["VectorMembers"]:        
        ...
        vecmems_readsio_impl  += implementations["vecmems_readsio"].format(type=klass,Num=str(vmcount))
        vecmems_writesio_impl += implementations["vecmems_writesio"].format(type=klass,Num=str(vmcount))
        vmcount = vmcount+1
