...
public:
  ExampleClusterSIOBlock()
    ...
    podio::SIOBlockFactory::instance().registerBlockForCollection(
        std::string("ExampleCluster"), this);
  }

  ExampleClusterSIOBlock(const std::string &nam)
      : // SIOBlock( nam+std::string("__ExampleCluster"),
  ...
  virtual void createCollection() override {
    setCollection(new ExampleClusterCollection);
  }

  SIOBlock *const create(const std::string name) override {
    return new ExampleClusterSIOBlock(name);
  }
};
