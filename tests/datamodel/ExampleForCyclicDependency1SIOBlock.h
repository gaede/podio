...
public:
  ExampleForCyclicDependency1SIOBlock()
      : SIOBlock("ExampleForCyclicDependency1",
                 sio::version::encode_version(0, 1)) {
    podio::SIOBlockFactory::instance().registerBlockForCollection(
        std::type_index(typeid(::ExampleForCyclicDependency1Collection)), this);
  }

  ExampleForCyclicDependency1SIOBlock(const std::string &nam)
      : SIOBlock(nam + std::string("__ExampleForCyclicDependency1"),
                 sio::version::encode_version(0, 1)) {}
  ...
  SIOBlock *const create(const std::string name) override {
    return new ExampleForCyclicDependency1SIOBlock(name);
  }
};

static ExampleForCyclicDependency1SIOBlock
    _dummyExampleForCyclicDependency1SIOBlock;
