...
public:
  ExampleWithStringSIOBlock()
    ...
    podio::SIOBlockFactory::instance().registerBlockForCollection(
        std::string("ExampleWithString"), this);
  }

  ExampleWithStringSIOBlock(const std::string &nam)
      : // SIOBlock( nam+std::string("__ExampleWithString"),
  ...
  virtual void createCollection() override {
    setCollection(new ExampleWithStringCollection);
  }

  SIOBlock *const create(const std::string name) override {
    return new ExampleWithStringSIOBlock(name);
  }
};
