...
public:
  ExampleWithVectorMemberSIOBlock()
      : SIOBlock("ExampleWithVectorMember",
                 sio::version::encode_version(0, 1)) {
    podio::SIOBlockFactory::instance().registerBlockForCollection(
        std::type_index(typeid(::ExampleWithVectorMemberCollection)), this);
  }

  ExampleWithVectorMemberSIOBlock(const std::string &nam)
      : SIOBlock(nam + std::string("__ExampleWithVectorMember"),
                 sio::version::encode_version(0, 1)) {}
  ...
  SIOBlock *const create(const std::string name) override {
    return new ExampleWithVectorMemberSIOBlock(name);
  }
};
